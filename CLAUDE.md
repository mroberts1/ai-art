...
This Obsidian vault is an LLM-managed knowledge base. The LLM writes and maintains all wiki content. The human rarely edits directly.
...
raw/            — Source documents ingested into the KB
  ...
  audiovisual/  — Notes on films, videos, and other AV sources
...
- Update `wiki/indexes/Main Index.md` and any relevant topic index pages with the new source and concepts
...
- Maintain `wiki/indexes/Main Index.md` as the central directory

### 3. Q&A (query → output)
When answering questions against the KB:
- First consult `wiki/indexes/Main Index.md` to find relevant articles
...
For example: `## [2026-06-23] ingest | Smith — "Generative Art and Machine Vision"`
...
- The Main Index links to all topic index pages
...
### Dataview
Used for live queries in the Health Dashboard.
...
- Manual setup: Settings → Templater → set Template folder to `templates`

### Tag Wrangler
Allows bulk rename and merge of tags across the vault. No configuration needed.

- The canonical tag vocabulary is in `wiki/indexes/Tag Vocabulary.md`
